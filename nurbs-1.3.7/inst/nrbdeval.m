...
% second derivatives
  ...
  % NURBS structure represents a surface
    temp = cw(ones(3,1),:,:);   % Weight of orinigal NURBS
    pnt = cp./temp;             % Evaluated point on the NURBS Surface times corresponding weight
  
    [cup,cuw] = nrbeval (dnurbs{1}, tt);    % Evaluated point of first derivative with respect to first input
    tempu = cuw(ones(3,1),:,:);             % Weight
    jac{1} = (cup-tempu.*pnt)./temp;
  
    [cvp,cvw] = nrbeval (dnurbs{2}, tt);    % Evaluated point of first derivative with respect to second input
    tempv = cvw(ones(3,1),:,:);             % Weight
    jac{2} = (cvp-tempv.*pnt)./temp;
